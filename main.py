...
def upload_to_stream(video_path, name):
    ...
    with open(video_path, "rb") as f:
        ...
        data = response.json()
        uid = data["result"]["uid"]
        return f"https://videodelivery.net/{uid}/manifest/video.m3u8"
...
def upload():
    ...
    # 3. Watermark Bas (FFmpeg)
    output_path = temp_video.name.replace(".mp4", "_out.mp4")
    ffmpeg_cmd = [
        "ffmpeg", "-i", temp_video.name, "-i", logo_path,
        "-filter_complex", "[1]scale=iw/2:-1[logo];[0][logo]overlay=(main_w-overlay_w)/2:main_h-overlay_h-80",
        "-codec:a", "copy", output_path
    ]
    subprocess.run(ffmpeg_cmd)

    # 4. Cloudflare'a Yükle ve Linki Al
    stream_url = upload_to_stream(output_path, name)
